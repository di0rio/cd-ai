...
Estado em 2026-09-15, sobre `main` em `024cc8e` mais o fechamento da Fase 5 (plano 015). Serve pra uma sessão/agente novo continuar o trabalho sem depender da conversa anterior.

- **Fontes da verdade:** [`AGENTS.md`](../AGENTS.md) (regras + comandos), [`SPEC.md`](../SPEC.md) (§34 define o roteiro), [`plans/README.md`](../plans/README.md) (status dos planos), [`docs/decisions/`](decisions/) (ADRs), [`docs/design/fase-4-tool-engine.md`](design/fase-4-tool-engine.md) (design da Fase 4), [`plans/015-fase-5-loop-ponta-a-ponta.md`](../plans/015-fase-5-loop-ponta-a-ponta.md) (plano da Fase 5).
...
Tudo em `main` (e o fechamento do 015 nesta árvore). Gate: `bun run verify`.
...
- **Núcleo Rust** ([`crates/agent-core/`](../crates/agent-core/)): workspace com validação de path/symlink, cliente Ollama com streaming cancelável, parser tolerante de tool calls, Tool Engine da Fase 4, e o loop da Fase 5:
  - `tools/` — `read_file`, `list_directory`, `search`, `edit_file`, `write_file`, `run_command`; `ToolEngine` (um por tarefa) com resolve → permissão → redata → evento; `CancelToken`; checkpoint SHA-256; busca literal via `fixed_strings(true)`.
...
## O que ainda falta (Fases 6–14)

| Fase | O que falta |
|---|---|
| **6 — Eval baseline** | Suíte `evals/` + runner na CLI; registrar taxa de sucesso inicial. O fixture `soma/` já existe. |
...
**Fase 6 (eval baseline)** — o loop já existe; o que falta é medir se um modelo real resolve o fixture (e os próximos) pela CLI, sem UI.
...
- O gate depende de `cargo` (edition 2024, Rust 1.85+) e `bun`. O aceite ao vivo depende do Ollama com o modelo CODER da decisão 0002.
